...
class WebMap{
    ...
    drawStaticPath = function(config){
        const refresh = config.refresh, showEntity = config.showEntity, debug = config.debug, color = config.color;
        if(debug){
        ...
        for(let i = 0; i < this.points.length - 1; i++){
            //pathPoints.push(new BMapGL.Point(this.points[i].getLongitude(), this.points[i].getLatitude()));
            this.map.addOverlay(new BMapGL.Polyline([new BMapGL.Point(this.points[i].getLongitude(), this.points[i].getLatitude()), new BMapGL.Point(this.points[i+1].getLongitude(), this.points[i+1].getLatitude())], {strokeColor: color, strokeWeight: 3, strokeOpacity: 0.8}));
        }
    ...
    additionalDrawStaticPath = function(newPoints, obj){
        const showEntity = obj.showEntity, debug = obj.debug, color = obj.color;
        if(debug){

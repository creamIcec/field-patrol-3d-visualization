...
wsPointServer.onmessage = function(event){
    ...
    for(let i = 0; i < points.length; i++){
        if(rawPathPoints.length === 0){
            ...
            point = new Point(points[i].longitude, points[i].latitude, points[i].rotation, points[i].time);
            const interpolatedPoints = getInterpolatedPoints(rawPathPoints[rawPathPoints.length - 1], point);
...
window.onload = () => {
    ...
    if(mapElement != null){
        ...
        //fetchVehicleName();
        initDragBehaviour(webMap.getMap(), document.querySelector("#header"), document.getElementsByClassName("button-wrapper"));
        isLoaded = true;
    }
